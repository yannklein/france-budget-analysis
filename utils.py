...
def identify_growth_trends(df: pd.DataFrame, min_years: int = 3) -> pd.DataFrame:
    ...
    for mission in df["Mission"].unique():
        ...
        volatility = (
            (mission_data["Montant"].std() / mission_data["Montant"].mean()) * 100
            if mission_data["Montant"].mean() > 0
            else 0
        )
...
def validate_budget_data(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    """
    validation = {"is_valid": True, "errors": [], "warnings": [], "stats": {}}
    ...
    if negative_amounts.any():
        validation["warnings"].append(
            f"{negative_amounts.sum()} negative budget amounts found"
        )
    ...
    validation["stats"] = {
        ...
        "total_budget_latest": (
            df[df["Année"] == df["Année"].max()]["Montant"].sum()
            if "Année" in df.columns and "Montant" in df.columns
            else 0
        ),
        "avg_mission_budget": (
...
def export_data_with_metadata(
    df: pd.DataFrame, predictions_df: pd.DataFrame = None
) -> str:
    """
    ...
    export_lines.append(
        f"# - Status: {'✓ Valide' if validation['is_valid'] else '✗ Erreurs détectées'}"
    )
    export_lines.append(
        f"# - Total enregistrements: {validation['stats']['total_records']}"
    )
    export_lines.append(
        f"# - Budget total (dernière année): {format_currency(validation['stats']['total_budget_latest'])}"
    )
    export_lines.append("")
    ...
    if predictions_df is not None:
        export_lines.append(
            "# - Montant_Prédit: Montant prédit (données futures uniquement)"
        )
...
def generate_insights(
    ...
    insights.append(
        f"📊 Analyse portant sur {total_years} années et {total_missions} missions budgétaires"
    )
    ...
    if total_years > 1:
        ...
        if total_growth > 0:
            insights.append(
                f"📈 Le budget total a augmenté de {total_growth:.1f}% entre {start_year} et {end_year} (croissance annuelle moyenne: {annual_growth:.1f}%)"
            )
        else:
            insights.append(
                f"📉 Le budget total a diminué de {abs(total_growth):.1f}% entre {start_year} et {end_year}"
            )
    ...
    if not top_missions.empty:
        ...
        insights.append(
            f"🏛️ En {latest_year}, '{top_mission['Mission']}' représente la plus grosse mission avec {format_currency(top_mission['Montant'])}"
        )
    ...
    if not trends.empty:
        ...
        if fastest_growing["Croissance_Annuelle_Pct"] > 5:
            insights.append(
                f"🚀 '{fastest_growing['Mission']}' affiche la plus forte croissance avec +{fastest_growing['Croissance_Annuelle_Pct']:.1f}% par an"
            )
        ...
        if not declining.empty:
            ...
            insights.append(
                f"📉 '{worst_decline['Mission']}' montre la plus forte baisse avec {worst_decline['Croissance_Annuelle_Pct']:.1f}% par an"
            )

    # Predictions insights
    if predictions_df is not None and not predictions_df.empty:
        pred_2030_total = predictions_df[predictions_df["Année"] == 2030][
            "Montant_Prédit"
        ].sum()
        ...
        if pred_growth > 0:
            insights.append(
                f"🔮 Les prédictions suggèrent une croissance du budget de {pred_growth:.1f}% d'ici 2030"
            )
        else:
            insights.append(
                f"🔮 Les prédictions suggèrent une baisse du budget de {abs(pred_growth):.1f}% d'ici 2030"
            )
        ...
        for mission in predictions_df["Mission"].unique():
            current_value = df[
                (df["Mission"] == mission) & (df["Année"] == df["Année"].max())
            ]["Montant"]
            pred_value = predictions_df[
                (predictions_df["Mission"] == mission)
                & (predictions_df["Année"] == 2030)
            ]["Montant_Prédit"]

            if not current_value.empty and not pred_value.empty:
                mission_growth = (
                    (pred_value.iloc[0] - current_value.iloc[0]) / current_value.iloc[0]
                ) * 100
        ...
        if pred_by_mission:
            ...
            if top_pred["Growth"] > 10:
                insights.append(
                    f"🎯 '{top_pred['Mission']}' devrait connaître la plus forte croissance avec +{top_pred['Growth']:.1f}% prédits d'ici 2030"
                )
...
def adjust_to_constant_euros(
    ...
    merged = df.merge(
        cpi_df[["Année", "CPI_Norm"]], on="Année", how="left", validate="many_to_one"
    )
    merged["CPI_Norm"] = merged["CPI_Norm"].replace({0: np.nan})
    merged["CPI_Norm"] = (
        merged["CPI_Norm"].fillna(method="ffill").fillna(method="bfill")
    )
...
_TRANSLATIONS: Dict[str, Dict[str, str]] = {
    ...
    "toggle.inflation": {
        ...
        "de": "Für Inflation anpassen (konstante Euro)",
        "es": "Ajustar por inflación (euros constantes)",
        "it": "Correggi per l'inflazione (euro costanti)",
        "pt": "Ajustar pela inflação (euros constantes)",
        "nl": "Aanpassen voor inflatie (constante euro's)",
        "pl": "Dostosuj do inflacji (stałe euro)",
    },
    "toggle.gov_periods": {
    ...
    "toggle.debt": {
        ...
        "pt": "Incluir juros da dívida",
        "nl": "Schuldrente opnemen",
        "pl": "Uwzględnij odsetki od długu",
    },
    # Tabs
    "tab.evolution": {
        "en": "Time Evolution",
        "fr": "Évolution Temporelle",
        "de": "Zeitliche Entwicklung",
        "es": "Evolución temporal",
        "it": "Evoluzione temporale",
        "pt": "Evolução temporal",
        "nl": "Tijdsevolutie",
        "pl": "Ewolucja w czasie",
    },
    "tab.compare": {
        "en": "Mission Comparison",
        "fr": "Comparaison Missions",
        "de": "Vergleich der Missionen",
        "es": "Comparación de misiones",
        "it": "Confronto missioni",
        "pt": "Comparação de missões",
        "nl": "Missievergelijking",
        "pl": "Porównanie misji",
    },
    "tab.split": {
        "en": "Budget Breakdown",
        "fr": "Répartition Budgétaire",
        "de": "Budgetaufteilung",
        "es": "Desglose del presupuesto",
        "it": "Ripartizione del budget",
        "pt": "Desagregação do orçamento",
        "nl": "Budgetverdeling",
        "pl": "Podział budżetu",
    },
    "tab.pred": {
    ...
    "title.app": {
        ...
        "de": "Französischer Staatshaushalt - Analyse und Vorhersagen",
        "es": "Presupuesto del Estado Francés - Análisis y Predicciones",
        "it": "Bilancio dello Stato francese - Analisi e Previsioni",
        "pt": "Orçamento do Estado Francês - Análise e Previsões",
        "nl": "Franse Rijksbegroting - Analyse en Voorspellingen",
        "pl": "Budżet Państwa Francuskiego - Analiza i Prognozy",
    },
    "desc.app": {
        ...
        "de": "Diese Anwendung analysiert die Entwicklung des französischen Staatshaushalts über 20 Jahre und bietet KI-basierte Vorhersagen.",
        "es": "Esta aplicación analiza la evolución del presupuesto estatal francés durante 20 años y proporciona predicciones basadas en IA.",
        ...
        "pt": "Esta aplicação analisa a evolução do orçamento do Estado francês ao longo de 20 anos e fornece previsões baseadas em IA.",
        ...
        "pl": "Ta aplikacja analizuje ewolucję budżetu państwa francuskiego na przestrzeni 20 lat i dostarcza prognozy oparte na sztucznej inteligencji.",
    },
}
...
def generate_insights_i18n(
    df: pd.DataFrame, predictions_df: pd.DataFrame | None, lang: str
) -> List[str]:
    """
    ...
    if df is None or df.empty:
        messages.append(
            translate(
                "insights.no_data", lang, "Aucune donnée disponible pour l'analyse."
            )
        )
        return messages
    ...
    header_tpl = translate(
        "insights.header",
        lang,
        "Analyse portant sur {years} années et {missions} missions budgétaires",
    )
        ...
        if start_total > 0:
            ...
            if total_growth >= 0:
                tpl = translate(
                    "insights.total_up",
                    lang,
                    "Le budget total a augmenté de {pct}% entre {start} et {end} (croissance annuelle moyenne: {avg}%)",
                )
                messages.append(
                    tpl.format(
                ...
                tpl = translate(
                    "insights.total_down",
                    lang,
                    "Le budget total a diminué de {pct}% entre {start} et {end}",
                )
                messages.append(
                    tpl.format(
                        pct=f"{abs(total_growth):.1f}", start=start_year, end=end_year
                    )
                )
    ...
    if top is not None and not top.empty:
        ...
        tpl = translate(
            ...
            lang,
            "En {year}, '{mission}' représente la plus grosse mission avec {amount}",
        )
        messages.append(
            tpl.format(
                year=latest_year,
    ...
    if predictions_df is not None and not predictions_df.empty:
        pred_2030_total = float(
            predictions_df[predictions_df["Année"] == 2030]["Montant_Prédit"].sum()
        )
        ...
        if current_total > 0:
            ...
            if pred_growth >= 0:
                tpl = translate(
                    "insights.pred_up",
                    lang,
                    "Les prédictions suggèrent une croissance du budget de {pct}% d'ici 2030",
                )
                ...
                tpl = translate(
                    "insights.pred_down",
                    lang,
                    "Les prédictions suggèrent une baisse du budget de {pct}% d'ici 2030",
                )
                messages.append(tpl.format(pct=f"{abs(pred_growth):.1f}"))

        by_mission = []
        for mission in predictions_df["Mission"].unique():
            curr = df[(df["Mission"] == mission) & (df["Année"] == latest_year)][
                "Montant"
            ]
            pred = predictions_df[
                (predictions_df["Mission"] == mission)
            ...
            if not curr.empty and not pred.empty and float(curr.iloc[0]) > 0:
                growth = (
                    (float(pred.iloc[0]) - float(curr.iloc[0]))
                    / float(curr.iloc[0])
                    * 100.0
                )
        ...
        if by_mission:
            ...
            if g > 10:
                tpl = translate(
                    "insights.pred_top",
                    lang,
                    "'{mission}' devrait connaître la plus forte croissance avec +{pct}% d'ici 2030",
                )
